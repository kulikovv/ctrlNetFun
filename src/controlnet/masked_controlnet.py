...
"""
I've created a simple class MaskedStableDiffusionControlNetPipeline that inherit all of its behaviour from  
StableDiffusionControlNetPipeline, except parameter mask, which is passed into __call__ function and is applied 
on the top of the output of the controlnet. Using this class we can use masked transformation for any type of 
conrolnet and stable diffusion models.
"""

import torch
from typing import Optional, Union, Callable, Dict, List, Any
from diffusers.pipelines.controlnet.pipeline_controlnet import StableDiffusionPipelineOutput, retrieve_timesteps
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            ...
            mask (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`):
                Apply spacialy 
        ...
        if callback is not None:
            deprecate(
                ...
                "Passing `callback` as an input argument to `__call__` is  \
                    deprecated, consider using `callback_on_step_end`",
            )
        if callback_steps is not None:
            deprecate(
                "callback_steps",
                "1.0.0",
                "Passing `callback_steps` as an input argument to `__call__` is \
                    deprecated, consider using `callback_on_step_end`",
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if mask is not None:
                    ...
                    mid_block_res_sample = apply_mask(
