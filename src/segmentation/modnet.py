...
class LRBranch(nn.Module):
    ...
    def __init__(self, backbone):
        ...
        self.conv_lr = Conv2dIBNormRelu(enc_channels[2], 1, kernel_size=3,
                                        stride=2, padding=1, with_ibn=False, with_relu=False)
...
class ModNetSegmentation:
    def __init__(self,
                 pretrained_ckpt: str = './pretrain/modnet_photographic_portrait_matting.ckpt') -> None:
        ...
        self.modnet = torch.nn.DataParallel(MODNet(backbone_pretrained=False))
    ...
    def __call__(self, image: PIL.Image.Image) -> torch.Tensor:
        image = self.torch_transforms(image).unsqueeze(0)
        _, _, matte = self.modnet(image.cuda() if torch.cuda.is_available() else image, True)
        return matte.cpu()
...
if __name__ == "__main__":
    ...
    md = ModNetSegmentation()

    matte = md(original_image)

    im = PIL.Image.fromarray(((matte[0, 0].numpy() * 255).astype('uint8')), mode='L')
    im.save("modnet_output.png")
